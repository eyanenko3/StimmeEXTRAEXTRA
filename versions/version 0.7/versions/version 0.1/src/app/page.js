...
export function renderApp(mountNode) {
  ...
  loopSteps.forEach((stepData, index) => {
    ...
    if (stepData.isDoNext) {
      ...
      const fullReportLink = document.createElement("div");
      fullReportLink.className = "full-report-cue";
      fullReportLink.innerHTML = `
        <span>Swipe up for full report</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <polyline points="18 15 12 9 6 15"></polyline>
        </svg>
      `;
      // Instead of opening on tap, show a hint
      fullReportLink.addEventListener("click", (e) => {
        showToast("Swipe up to read full report");
      });
  ...
  // Scroll down past all info to open article drawer (swipe up gesture)
  let drawerOpening = false;
  
  // 1. Regular scroll event (when content overflows and scrolls to bottom)
  contentArea.addEventListener("scroll", () => {
    if (loopSteps[activeStepIndex].isDoNext) return;
    const isAtBottom = contentArea.scrollHeight - contentArea.scrollTop - contentArea.clientHeight <= 10;
    if (isAtBottom && !drawerOpening) {
      drawerOpening = true;
      openArticleDrawer();
      setTimeout(() => { drawerOpening = false; }, 800);
    }
  });

  // 2. Touch swipe up (dragging finger up / scrolling down) when already at bottom
  let touchStartY = 0;
  ...
  contentArea.addEventListener("touchmove", (e) => {
    ...
    const diffY = touchStartY - touchY; // positive when dragging up (scrolling down)
    const isAtBottom = contentArea.scrollHeight - contentArea.scrollTop - contentArea.clientHeight <= 10;
    
    if (isAtBottom && diffY > 45 && !drawerOpening) {
      drawerOpening = true;
      openArticleDrawer();
      setTimeout(() => { drawerOpening = false; }, 800);
    }
  ...
  // 3. Mouse wheel scroll down when already at bottom
  contentArea.addEventListener("wheel", (e) => {
    if (loopSteps[activeStepIndex].isDoNext) return;
    const isAtBottom = contentArea.scrollHeight - contentArea.scrollTop - contentArea.clientHeight <= 10;
    if (isAtBottom && e.deltaY > 15 && !drawerOpening) {
      drawerOpening = true;
      openArticleDrawer();
      setTimeout(() => { drawerOpening = false; }, 800);
    }
      ...
      <div class="completion-card" id="completionCard">
        ...
        <div class="completion-card-inner">
          <div class="completion-icon-wrapper">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="completion-check">
              <polyline points="20 6 9 17 4 12"></polyline>
            </svg>
          </div>
          <h3 class="completion-title">You're All Caught Up!</h3>
          <p class="completion-subtitle">Thank you for staying informed with Stimme Loops.</p>
        </div>
      </div>
    </div>
  `;
  ...
  const completionCard = articleDrawer.querySelector("#completionCard");
  drawerContent.addEventListener("scroll", () => {
    const threshold = drawerContent.scrollTop + drawerContent.clientHeight;
    const cardTop = completionCard.offsetTop;
    if (threshold >= cardTop + 20) {
      completionCard.classList.add("is-finished");
    }
  });

  function openArticleDrawer() {
    if (completionCard) completionCard.classList.remove("is-finished");
    articleDrawer.style.transform = "";
